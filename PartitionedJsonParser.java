...
//import static JsonLexer.JsonLexerState;
...
public class PartitionedJsonParser {
    ...
    private static final EnumSet<JsonLexer.JsonLexerState> inStringStates =
            EnumSet.of(JsonLexer.JsonLexerState.INSIDE_STRING, JsonLexer.JsonLexerState.STRING_ESCAPE);
    ...
    public String nextObjectContainingMember(String member) throws IOException {
        ...
        while ((i = inputStreamReader.read()) != -1) {
            ...
            switch (memberState) {
                case SEARCHING:
                    if (lexer.getState() == JsonLexer.JsonLexerState.BEGIN_STRING) {
                    ...
                    } else if (lexer.getState() == JsonLexer.JsonLexerState.END_STRING && member.equals(currentString.toString())) {
                    ...
                    } else if (lexer.getState() == JsonLexer.JsonLexerState.BEGIN_OBJECT) {
                    ...
                    } else if (lexer.getState() == JsonLexer.JsonLexerState.END_OBJECT) {
                        if (objectStack.size() > 0) {
                ...
                case FOUND_STRING_NAME:
                    ...
                    if (lexer.getState() != JsonLexer.JsonLexerState.WHITESPACE) {
                        if (lexer.getState() == JsonLexer.JsonLexerState.NAME_SEPARATOR) {
                            // found our member!
                            //
                ...
                case IN_MATCHING_OBJECT:
                    if (lexer.getState() == JsonLexer.JsonLexerState.BEGIN_OBJECT) {
                        objectCount++;
                    } else if (lexer.getState() == JsonLexer.JsonLexerState.END_OBJECT) {
                        objectCount--;
                        if (objectCount < 0) {
